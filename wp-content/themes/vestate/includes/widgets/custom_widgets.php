...
function vibe_widgets() {
    ...
    register_widget('Vibe_Advanced_Listing_Search');
    register_widget('vibefeaturedlistings');
    register_widget('vibeloancalculator');
    register_widget('vibecurrencyconvertor');
}
...
function form($instance) {
    ...
    $defaults = array( 
                                ...
                                'num_tab1' => '10', 
                                'show_tab2'  => '1',
                                'title_tab2'  => __('Recent Comments','vibe'),
                                'style_tab2' => 'thumbnail', //thumbnail, normal, carousel, Thumbnail grid
                                'type_tab2'  => 'post', //post,project,testimonials
                                'sort_tab2'  => 'recent',  //Recent , Recent Tags, Recent Category, 
                                'num_tab2' => '10', 
                                'show_tab3'  => '1',
                                'title_tab3'  => __('Recent Projects','vibe'),
    ...
    <p>
                ...
                <p> <?php _e('Title','vibe'); ?> : <input class="text" type="text" name="<?php echo $this->get_field_name('title_tab'.$k); ?>" value="<?php echo $instance['title_tab'.$k]; ?>" />
                </p><p><?php _e('Style','vibe');?> : <select class="select" name="<?php echo $this->get_field_name('style_tab'.$k); ?>">
                        <option value="thumbnail" <?php if($instance['style_tab'.$k] == 'thumbnail') { ?>selected="selected" <?php } ?>><?php _e('Thumbnail Posts','vibe'); ?></option>
                        <option value="grid" <?php if($instance['style_tab'.$k] == 'grid') { ?>selected="selected" <?php } ?>><?php _e('Thumbnail Grid','vibe'); ?></option>
                        <option value="list" <?php if($instance['style_tab'.$k] == 'list') { ?>selected="selected" <?php } ?>><?php _e('Default List','vibe'); ?></option>
                        </select>
                </p><p><?php _e('Type','vibe'); ?> : <select class="select" name="<?php echo $this->get_field_name('type_tab'.$k); ?>">
                        <option value="post" <?php if($instance['type_tab'.$k] == 'post') { ?>selected="selected" <?php } ?>><?php _e('Post','vibe'); ?></option>
                        <?php
                        $args=array(
                                    'public'   => true,
                                    '_builtin' => false
                        ...
                        <option value="<?php echo $post_type; ?>" <?php if($instance['type_tab'.$k] == $post_type) { ?>selected="selected" <?php } ?>><?php echo $post_type; ?></option>;
                        <?php }
                        ?>
                </select>
                </p><p><?php _e('Sort','vibe'); ?> : <select class="select" name="<?php echo $this->get_field_name('sort_tab'.$k); ?>">
                        <option value="recent" <?php if($instance['sort_tab'.$k] == 'recent') { ?>selected="selected" <?php } ?>><?php _e('Recent','vibe'); ?></option>
                        <option value="popular" <?php if($instance['sort_tab'.$k] == 'popular') { ?>selected="selected" <?php } ?>><?php _e(' Popular','vibe'); ?></option>
                        </select>
                </p><p><?php _e('Number of items','vibe');?> : <input class="text" type="text" name="<?php echo $this->get_field_name('num_tab'.$k); ?>" value="<?php echo $instance['num_tab'.$k]; ?>" />
    </p>  
                    <?php
                }//End Tabs
  }
...
function update($new_instance, $old_instance) {
    ...
    $instance['theme']=$new_instance['theme'];
               ...
               for($k=1;$k<4;$k++){ 
                        ...
                        if(isset( $new_instance['type_tab'.$k] ))
                        $instance['type_tab'.$k]=$new_instance['type_tab'.$k];
                        if(isset( $new_instance['sort_tab'.$k] ))
                        $instance['sort_tab'.$k]=$new_instance['sort_tab'.$k];
    ...
    <div class="tab-content <?php echo $instance['theme'];?>">
            ...
            for($i=1;$i<4;$i++){
                ...
                if($i==1) echo '<div class="tab-pane active" id="tab'.$i.'">';
                    ...
                    if($instance['type_tab'.$i] == 'post'){
                                    if($instance['sort_tab'.$i] == 'recent') {
                                            $query = 'posts_per_page='.$instance['num_tab'.$i];
                                            $loop = new WP_Query($query);
                                    }else{
                                        $query='orderby=comment_count&posts_per_page='.$instance['num_tab'.$i];
                                        $loop = new WP_Query($query);
                                        }
                       }else{
                        
                                            $query = 'post_type='.$instance['type_tab'.$i].'&posts_per_page='.$instance['num_tab'.$i];
                                            $loop = new WP_Query($query);
                            }
                  
                    
                      if($instance['style_tab'.$i] == 'thumbnail'){ 
                           echo '<ul class="more_posts">';
                            while ( $loop->have_posts() ) : $loop->the_post();
                                ...
                                if(isset($price)){
                                    echo '<span class="price"><i class="'.$vibe_options['currency'].'"></i>'.$price.'</span>';
                                    echo get_the_term_list( $loop->post->ID, 'location', ' ', ' ', '' );
                                }
          ...
          echo '</small></li>';
                      ...
                      if($instance['style_tab'.$i] == 'grid' ){ 
                           echo '<ul class="grid">';
                            while ( $loop->have_posts() ) : $loop->the_post();   
                                echo '<li><a href="'.get_permalink($loop->post->ID).'" title="'.get_the_title($loop->post->ID).'">'.get_the_post_thumbnail($loop->post->ID,'mini').'</a>';
          echo '</li>';
                            endwhile;
                           echo '</ul>';
                        // Reset Post Data
                      ...
                      if($instance['style_tab'.$i] == 'list'){ 
                           echo '<ul class="list">'; 
                             while ( $loop->have_posts() ) : $loop->the_post();   
                                echo '<li><a href="'.get_permalink($loop->post->ID).'" title="'.get_the_title($loop->post->ID).'">'.get_the_title().'</a>';
          echo '</li>';
                            endwhile;
                           echo '</ul>';
                        // Reset Post Data
...
//* ==== VIBE LOAN CALCULATOR ==== */

class vibeloancalculator extends WP_Widget {

  function vibeloancalculator() {
    $widget_ops = array( 'classname' => 'Loan Calculator', 'description' => __('Vibe Loan Calculator', 'vibe') );
    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'vibeloancalculator' );
    $this->WP_Widget( 'vibeloancalculator', __('Vibe Loan Calculator', 'vibe'), $widget_ops, $control_ops );
  }
  
  function widget( $args, $instance ) {
    extract( $args );
    
    $currency=vibe_get_option('currency');
    //Our variables from the widget settings.
    $title = apply_filters('widget_title', $instance['title'] );
    ...
    if ( $title )
      echo $before_title . $title . $after_title;
      ?>
        <input type="number" id="loan_amount" class="form_field" placeholder="Loan Amount " />
        <input type="number" id="loan_years" class="form_field" placeholder="Loan Term (in Years) " />
        <input type="text" id="loan_interest" class="form_field" placeholder="Interest Rate (in %) " />
        <a href="#" id="calculate_loan" class="btn" data-currency="<?php echo $currency; ?>">Calculate</a>
      <?php
      echo $after_widget;
    } // END WIDGET Function
  ...
  function form( $instance ) {
    //Set up some default widget settings.
    $defaults = array( 'title'  => 'Loan Calculator',
                        );
    $instance = wp_parse_args( (array) $instance, $defaults ); 
    ?>
    <p> <?php _e('Title','vibe'); ?> <input type="text" class="text" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" /></p>
...
class vibecurrencyconvertor extends WP_Widget {

  function vibecurrencyconvertor() {
    $widget_ops = array( 'classname' => 'Vibe Currency Calculator', 'description' => __('Vibe Currency Convertor', 'vibe') );
    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'vibecurrencyconvertor' );
    $this->WP_Widget( 'vibecurrencyconvertor', __('Vibe Currency Convertor', 'vibe'), $widget_ops, $control_ops );
  }
  ...
  function widget( $args, $instance ) {
    ...
    if ( $title )
      ...
      $currencies = vibe_currency_symbols();
      $base_currency=vibe_get_option('currency');


      ?>
       <ul class="currency_conversion">
          <li><a class="active" data-conversion="1" data-rel="tooltip" title="<?php echo $currencies[$base_currency]; ?>"><i class="<?php echo $base_currency; ?>"></i></a></li>
          <li><a data-rel="tooltip" data-conversion="<?php echo $conversion; ?>" title="<?php echo $currencies[$currency]; ?>"><i class="<?php echo $currency; ?>"></i></a></li>
       </ul> 
      <?php
      echo $after_widget;
    } // END WIDGET Function
  ...
  function update( $new_instance, $old_instance ) {
    ...
    $instance['conversion'] = strip_tags( $new_instance['conversion'] );
  ...
  function form( $instance ) {
    //Set up some default widget settings.
    $defaults = array( 'title'  => 'Currency Converter',
                        );
    $instance = wp_parse_args( (array) $instance, $defaults ); 
    $base_currency=vibe_get_option('currency');
    ?>
    <p> <?php _e('Title','vibe'); ?> <input type="text" class="text" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" /></p>
    <p> <?php _e('Currency to Convert to from base currency : ','vibe'); 
    $currencies = vibe_currency_symbols();
    echo '<strong>'.$currencies[$base_currency].'</strong> to ';
    ?>
    <select name="<?php echo $this->get_field_name('currency'); ?>">
    <?php
    foreach($currencies as $key=>$value){
        echo '<option value="'.$key.'" '.selected($key,$instance['currency']).'>'.$value.'</option>';
    }
     ?> 
    </select> 
    </p>
    <p><?php _e('Conversion Rate :','vibe'); ?>
    <input type="text" name="<?php echo $this->get_field_name('conversion'); ?>" value="<?php echo $instance['conversion']; ?>"/>
    </p>
   <?php
  }
}
