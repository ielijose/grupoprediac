...
class vibe_woocommerce_carousels extends WP_Widget {
    ...
    function widget( $args, $instance ) {
		...
		if ( $title )
			echo '<h3 class="heading"><span>' . $title . '</span></h3>';
                ...
                if(isset($instance['scroll']) && $instance['scroll'] == 'vertical'){
                    global $vibe_options;
                    $height = $instance['columns']*($vibe_options['woocommerce_product_height']+30);
                    echo '<style>
                        #'.$rand.' .flex-viewport{height:'.$height.'px !important;}</style>';
                    
                }
               
               echo '<div id="'.$rand.'" class="vibe_carousel flexslider '.$instance['controls'].' '.$instance['scroll'].'  columns'.$instance['columns'].'">'
	...
	<p>
        ...
        <p>
          <label for="<?php echo $this->get_field_id('scroll'); ?>"><?php _e('Carousel Scroll Direction','vibe'); ?></label> 
           <select class="select" name="<?php echo $this->get_field_name('scroll'); ?>">
               <option value="horizontal" <?php selected($instance['scroll'],'horizontal'); ?>>Horizontal Scroll</option>              
               <option value="vertical" <?php selected($instance['scroll'],'vertical'); ?>>Vertical Scroll</option>                                   
           </select>
        </p>
        <p>
