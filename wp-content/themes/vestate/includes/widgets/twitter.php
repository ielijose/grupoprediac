...
protected function really_simple_twitter_messages($options) {
		...
		if (!isset($options['skip_text']) or $options['skip_text']!='' or !isset($options['skip_replies']) or $options['skip_replies'] or !isset($options['skip_retweets']) or $options['skip_retweets']) {
		...
		$transient_name = 'twitter_data_'.$options['username'].(isset($options['skip_text'])?$options['skip_text']:'').'_'.$options['num'];
		...
		if (isset($options['erase_cached_data']) && $options['erase_cached_data']) {
			...
			if (isset($twitter_data['errors'])) {
			    ...
			    if(false === ($twitter_data = get_option($transient_name."_valid"))) {
				    ...
				    return __('Unable to get tweets'.$debug,'vibe');
				}
			}
		}
		...
		foreach($twitter_data as $message) {
			...
			if (isset($options['skip_text']) and $options['skip_text']!='' and strpos($msg, $options['skip_text'])!==false) {
				continue;
			}
			...
			// TODO: LINK
			if (isset($options['thumbnail']) and $options['thumbnail'] and $message['user']['profile_image_url_https']!='') {
