...
jQuery(document).ready(function(){
  ...
  jQuery('.add_attribute').click(function(e){
      var newp = jQuery('#newp').clone();
       newp.removeAttr('style');
       newp.removeAttr('id');
       newp.find('input').val('');
       var name = newp.find('input').attr('rel-name');
       var n = parseInt(jQuery('#number_n').val());
        n = n+1;
    
    alert(n);
       jQuery('#number_n').val(n);
       name = name.replace('00',n);
       newp.find('input').attr('name' , name);
       
       var sname=newp.find('select').attr('rel-name')
       sname = sname.replace('00',n);
       newp.find('select').attr('name' , sname );
       
       jQuery(this).parent().prepend(newp);	
       return false;
  });
  jQuery('.removelisting_attribute').live('click',function(e){
     jQuery(this).parent().fadeOut('slow', function(){jQuery(this).remove();});
      return false;
  });
});

//For Advanced Search Widget
jQuery(document).ready(function($){
    $('body').delegate('#generate_tree','click',function(event){
            event.preventDefault();

            var $this=$(this);
            var text=$this.html();
            var tab=$this.parent().find('select').val();


            $this.html('<i class="icon-spinner"></i> Generating tree...');
            $.ajax({
                  type: "POST",
                  url: ajaxurl,
                      data: {   action: 'generate_taxtree', 
                                tax:tab
                            },
                  cache: false,
                  success: function (html) {
                      $this.html(html);
                      setTimeout(function(){$this.html(text);}, 5000);
                  }
              });
    });
});
