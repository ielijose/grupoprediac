...
/**
 * FILE: customizer.php 
 * Created on Mar 1, 2013 at 7:07:46 PM 
 * Author: Mr.Vibe 
...
function vibe_customizer_setup() {
  $customize = get_option('viz_customizer');
  if(!isset($customize)){
      add_option('viz_customizer','');
  }
}

// add some settings and such

add_action('customize_register', 'vibecom_customize');
add_action('after_setup_theme','vibe_customizer_setup');



function vibecom_customize($wp_customize) {
...
/*====================================================== */
    $i=164;
    foreach($viz_customizer['sections'] as $key=>$value){
...
/*====================================================== */
    ...
    foreach($viz_customizer['controls'] as $section => $settings){ $i=1;
        foreach($settings as $control => $type){
            $i=$i+2;
            /*====== REGISTER SETTING =========*/
            $wp_customize->add_setting( 'viz_customizer['.$control.']', array(
                                                'label'         => $type['label'],
                                                'type'           => 'option',
            ...
            switch($type['type']){
                case 'color':
                        ...
                        'label'   => $type['label'],
                        'section' => $section,
                        'settings'   => 'viz_customizer['.$control.']',
                        'priority'       => $i
                        ) ) );            
                    break;
                case 'image':
